...
router.get("/", (req, res) => {
  ...
  con.db.query(qrySet, (err, result) => {
    ...
    const tourQry = "SELECT tournaam, jaar,Date_Format(startdatum,'%W %e %M') as startdatum,	Date_Format(einddatum,'%W %e %M') as einddatum,	nummer,	Date_Format(datum,'%W %e %M') as datum,	van,	naar, 	afstand,	rittype FROM tours t JOIN etappes e ON e.tour_id = t.id"
